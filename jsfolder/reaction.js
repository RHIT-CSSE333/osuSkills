...
function PatternReq(p1, p2, p3, CSpx)
{
	...
	let point1 = Vector2d(p1.pos.X, p1.pos.Y);
	let point2 = Vector2d(p2.pos.X, p2.pos.Y);
	let point3 = Vector2d(p3.pos.X, p3.pos.Y);
...
{
	let i = FindHitobjectAt(_hitobjects, _currTime);
...
{
	let tickPoint;
	let i = FindHitobjectAt(_hitobjects, _time, true);
...
{
	let damping = GetVar("Reaction", "PatternDamping");		// opposite of sensitivity; how much the patterns' influence is damped
...
function react2Skill(_timeToReact)
{
	...
	let a = pow(2.0, log(78608.0 / 15625.0) / log(34.0 / 25.0))*pow(125.0, log(68.0 / 25.0) / log(34.0 / 25.0));
	let b = log(2.0) / (log(2.0) - 2.0*log(5.0) + log(17.0));
...
{
	let timeToReact = 0.0;
	let FadeInReactReq = GetVar("Reaction", "FadeinPercent"); // players can react once the note is 10% faded in
...
function CalculateReaction(beatmap, hidden)
{
	...
	let lastObj = beatmap.hitObjects[beatmap.hitObjects.size() - 1].time;

    beatmap.targetPoints.forEach(myFunc);

    function myFunc(ticks) {
